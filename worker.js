...
async function handleMobileAwareAsset(request, env) {
  ...
  return responsePatchedHtml(asset, html, 'x-lnb-mobile-shell', 'v4');
}
...
function injectMobileAppShell(html, request, moduleName, forceMobile) {
  ...
  const marker = 'data-lnb-mobile-shell="v4"';
  if (html.includes(marker)) return html;
  const moduleId = String(moduleName || 'modulo').replace(/[^a-z0-9_-]/gi, '').toLowerCase();
  const style = '<link rel="stylesheet" href="/runtime-patches/mobile-app-shell.css?v=4" '+marker+'>';
  const script = '<script src="/runtime-patches/mobile-app-shell.js?v=4" data-lnb-mobile-module="'+moduleId+'" '+marker+' defer></' + 'script>';
  let out = html.replace(/<html(\s[^>]*)?>/i, function(match, attrs){
    const rest = attrs || '';
    return '<html'+rest+' data-lnb-mobile-shell="v4" data-lnb-mobile-module="'+moduleId+'">';
  });
  if (out.includes('</head>')) out = out.replace('</head>', style + '\n</head>'); else out = style + '\n' + out;
  if (out.includes('</body>')) out = out.replace('</body>', script + '\n</body>'); else out += '\n' + script;
  return out;
