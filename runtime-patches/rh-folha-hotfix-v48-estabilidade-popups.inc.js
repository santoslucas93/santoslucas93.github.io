...
function provisionPopup48(card,kind){
  ...
  if(label.indexOf('saldo provisionado')>=0){
    h=['Colaborador','Departamento','CC','Saldo atual'];body=rows.map(function(r){return[r.name,r.dep,r.cc,money48(r.saldo)]});foot=['TOTAL','','',money48(sum48(rows,'saldo'))];sub='Composição exclusiva do saldo provisionado. Encargos e custo total não são somados neste indicador.';
  }else if(label.indexOf('provisao do mes')>=0){
    h=['Colaborador','Departamento','CC','Provisão do mês'];body=rows.map(function(r){return[r.name,r.dep,r.cc,money48(r.pm)]});foot=['TOTAL','','',money48(sum48(rows,'pm'))];sub='Composição da provisão reconhecida no mês.';
  ...
  var widths=h.length===4?[38,24,16,22]:h.length===5?[34,22,15,14,15]:h.length===7?[30,12,9,12,11,9,17]:null;
...
function styles48(){
  ...
  '.kpi,.rh40-guide-card,.rh41-report-card,.rh-close-stat,.rh-history-metric,.summary-card,.stat-card,.rh47-summary-card{min-width:0!important;backface-visibility:hidden!important}'+
  '.kpi strong,.rh40-guide-card strong,.rh41-report-card strong,.rh-close-stat strong,.rh-history-metric strong,.summary-card strong,.stat-card strong,.rh47-summary-card strong{visibility:visible!important;opacity:1!important;min-height:1.08em!important;font-variant-numeric:tabular-nums!important;animation:none!important;transition:none!important}'+
  '#page-planejamento .kpi{min-height:104px}#page-planejamento .kpi strong{display:block!important}'+
  ...
  '.rh48-modal-card{width:min(1180px,calc(100vw - 44px));max-height:calc(100vh - 44px);display:flex;flex-direction:column;overflow:hidden;border:1px solid var(--line);border-radius:18px;background:var(--surface);color:var(--text);box-shadow:0 30px 80px rgba(0,0,0,.42)}'+
  ...
  '.rh48-modal-body{min-height:0;overflow:auto;padding:14px 22px 22px}.rh48-table-scroll{width:100%;overflow:auto;border:1px solid var(--line-soft);border-radius:12px}.rh48-table{width:100%!important;min-width:640px!important;table-layout:fixed!important;border-collapse:collapse!important}.rh48-table th,.rh48-table td{padding:10px 11px!important;border-bottom:1px solid var(--line-soft)!important;vertical-align:middle!important;overflow:hidden!important;text-overflow:ellipsis!important;white-space:nowrap!important;transition:none!important;animation:none!important}.rh48-table th{background:var(--surface-2)!important;color:var(--muted)!important;font-size:.66rem!important;text-transform:uppercase!important;letter-spacing:.055em!important}.rh48-table td{font-size:.76rem!important}.rh48-table td.money,.rh48-table th.money{text-align:right!important;font-variant-numeric:tabular-nums!important}.rh48-table tfoot td{background:color-mix(in srgb,var(--surface-2) 82%,var(--gold) 8%)!important;border-top:2px solid var(--line)!important;border-bottom:0!important;font-weight:900!important}.rh48-table tbody tr:last-child td{border-bottom:0!important}'+
  '@media(max-width:760px){.rh48-overlay{padding:8px}.rh48-modal-card{width:calc(100vw - 16px);max-height:calc(100vh - 16px)}.rh48-modal-head{padding:16px}.rh48-sub,.rh48-formula{margin-left:16px;margin-right:16px}.rh48-sub{padding-left:0;padding-right:0}.rh48-modal-body{padding:12px 16px 16px}.rh48-table{min-width:760px!important}}';
