...
async function exportExcel57(){
  ...
  var wp=wb.addWorksheet('Colaboradores');wp.views=[{showGridLines:false,state:'frozen',ySplit:1}];header(wp.addRow(['Colaborador','Departamento','Vínculo','Salário-base','Dias férias','Proventos','INSS','IRRF','Outros descontos','Líquido','INSS patronal','RAT','Terceiros','PIS','FGTS','Custo']));t.rows.forEach(function(r){wp.addRow([r.nome,r.departamento,r.vinculo,r.salary,r.vacDays,r.proventos,r.inssEmp,r.irrf,r.opsTotal+r.pension+r.manualDiscount,r.liquido,r.inssPat,r.rat,r.terceiros,r.pis,r.fgts,r.custo])});wp.columns.forEach(function(c,i){c.width=i===0?38:(i<3?20:15);if(i>=3)c.numFmt='R$ #,##0.00'});
