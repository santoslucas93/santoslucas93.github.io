...
function benefit57(p){try{var b=typeof rhPersonBenefit==='function'?rhPersonBenefit(p):{};return n57(b.seguro_vida)+n57(b.assistencia_medica||b.assist_medica)+n57(b.vr_caixa||b.vr_va_cesta)+n57(b.vale_transporte)}catch(e){return 0}}
...
function row57(ctx){
  ...
  var inssPat=trunc257(employerBase*rr.inssPat),rat=trunc257(employerBase*rr.rat),terc=trunc257(employerBase*rr.terceiros),pis=trunc257(employerBase*rr.pis),fgts=trunc257(fgtsBase*fgtsRate),company=r257(inssPat+rat+terc+pis+fgts),ben=r257(benefit57(p)),cost=r257(gross+company+ben);
  ...
  return{ctx:ctx,person:p,id:personKey57(p),folhaId:String(p.id||''),nome:p.nome||'—',departamento:dep,vinculo:p.vinculo_snapshot||p.vinculo||'—',salary:r257(salary),workRatio:workRatio,vacDays:vacDays,cashDays:cashDays,vacPrev:vacPrev,recurring:rec,recurringTotal:recTotal,baseSalary:base,vacationPay:vacationPay,vacationThird:vacationThird,vacationGross:vacationGross,cashGross:cashGross,vacationAdvance:vacationAdvance,regularGross:regularGross,employerBase:employerBase,fgtsBase:fgtsBase,proventos:gross,ops:ops,opsTotal:opsTotal,manualDiscount:manual,pension:pension,dependents:deps,inferredDependents:inferred,otherDeduct:otherDeduct,inssEmp:inssEmp,vacationInss:vacationInss,regularInss:regularInss,irrf:irrfTotal,regularIrrf:regularIrrf.value,vacationIrrf:vacationIrrf.value,irrfBase:r257(regularIrrf.base+vacationIrrf.base),irrfDeduction:r257(regularIrrf.deduction+vacationIrrf.deduction),irrfSimplified:regularIrrf.simplified&&vacationIrrf.simplified,irrfReduction:r257(regularIrrf.reduction+vacationIrrf.reduction),retained:r257(inssEmp+irrfTotal),descontos:discounts,liquido:net,inssPat:inssPat,rat:rat,terceiros:terc,pis:pis,fgts:fgts,encargos:company,taxTotal:r257(inssEmp+irrfTotal+company),beneficios:ben,custo:cost,isClt:clt,isIntern:intern,hasInss:hasInss,hasFgts:hasFgts,param:param}
}
function snapshot57(){
  var rows=V57.contexts.map(row57).sort(function(a,b){return b.custo-a.custo}),t={rows:rows};
  ['proventos','descontos','liquido','inssEmp','irrf','inssPat','rat','terceiros','pis','fgts','encargos','beneficios','custo'].forEach(function(k){t[k]=sum57(rows,k)});
...
function table57(t){
  ...
  return '<div class="table-wrap"><table class="rh57-table"><thead><tr><th>Colaborador</th><th>Departamento</th><th class="money">Salário-base</th><th class="money">Proventos previstos</th><th class="money">Descontos calculados</th><th class="money">Líquido previsto</th><th class="money">Encargos empresa</th><th class="money">Benefícios</th><th class="money">Custo previsto</th></tr></thead><tbody>'+t.rows.map(function(r){return '<tr data-rh57-row="'+esc57(r.id)+'"><td><b>'+esc57(r.nome)+'</b><small class="rh57-row-meta">'+esc57(r.vinculo)+(r.vacPrev?' · férias na base anterior':'')+(r.vacDays?' · '+r.vacDays+' dia(s) de férias projetados':'')+'</small>'+(admin?'<button type="button" class="rh57-mini" data-rh57-adjust="'+esc57(r.id)+'">Ajustar cálculo</button>':'')+'</td><td>'+esc57(r.departamento)+'</td><td class="money">'+money57(r.salary)+'</td><td class="money">'+money57(r.proventos)+'</td><td class="money">'+money57(r.descontos)+'</td><td class="money"><b>'+money57(r.liquido)+'</b></td><td class="money">'+money57(r.encargos)+'</td><td class="money">'+money57(r.beneficios)+'</td><td class="money"><b>'+money57(r.custo)+'</b></td></tr>'}).join('')+'</tbody><tfoot><tr><td><b>TOTAL</b></td><td></td><td></td><td class="money"><b>'+money57(t.proventos)+'</b></td><td class="money"><b>'+money57(t.descontos)+'</b></td><td class="money"><b>'+money57(t.liquido)+'</b></td><td class="money"><b>'+money57(t.encargos)+'</b></td><td class="money"><b>'+money57(t.beneficios)+'</b></td><td class="money"><b>'+money57(t.custo)+'</b></td></tr></tfoot></table></div>'
}
...
function installSummary57(t){
  ...
  box.innerHTML='<div class="rh47-summary-grid">'+
    ...
    summaryCard57('Benefícios',t.beneficios,'fonte integrada da competência-base','ben')+
    summaryCard57('Tributos / recolhimentos',t.taxTotal,'retidos + encargos da empresa','tax')+
    summaryCard57('Custo total estimado',t.custo,'proventos + encargos + benefícios','cost',true)+
...
function taxMetric57(key){
  var map={INSS_EMP:['INSS dos segurados','inssEmp'],IRRF:['IRRF sobre folha','irrf'],INSS_PAT:['INSS patronal','inssPat'],RAT:['RAT','rat'],TERC:['Terceiros','terceiros'],PIS:['PIS sobre folha','pis'],FGTS:['FGTS','fgts']},x=map[key];if(!x)return;metricModal57(x[0],V57.snapshot.rows,x[1],V57.snapshot[x[1]],key==='IRRF'?'IRRF calculado individualmente com a melhor dedução entre a legal e o desconto simplificado, seguido da redução mensal de 2026.':'Cálculo por colaborador; o total é a soma das parcelas já arredondadas/truncadas em centavos.')
}
function handleCard57(key){var t=V57.snapshot;if(!t)return;if(key==='prov')return metricModal57('Proventos previstos',t.rows,'proventos',t.proventos);if(key==='disc')return metricModal57('Descontos calculados',t.rows,'descontos',t.descontos);if(key==='liq')return metricModal57('Líquido previsto',t.rows,'liquido',t.liquido);if(key==='ret')return metricModal57('Impostos retidos',t.rows,'retained',t.retained,'INSS dos segurados + IRRF, conciliados por colaborador.');if(key==='company')return metricModal57('Encargos da empresa',t.rows,'encargos',t.encargos);if(key==='ben')return metricModal57('Benefícios',t.rows,'beneficios',t.beneficios);if(key==='tax')return metricModal57('Tributos / recolhimentos',t.rows,'taxTotal',t.taxTotal,'Retenções dos colaboradores + encargos da empresa, conciliados por colaborador.');if(key==='cost')return metricModal57('Custo total estimado',t.rows,'custo',t.custo)}
...
async function exportPdf57(){
  ...
  doc.autoTable({startY:36,head:[['Indicador','Valor']],body:[['Proventos previstos',money57(t.proventos)],['Descontos calculados',money57(t.descontos)],['Líquido previsto',money57(t.liquido)],['Impostos retidos',money57(t.retained)],['Encargos da empresa',money57(t.encargos)],['Benefícios',money57(t.beneficios)],['Custo total estimado',money57(t.custo)]],theme:'grid',headStyles:{fillColor:[13,43,66]},columnStyles:{1:{halign:'right',fontStyle:'bold'}}});
  doc.autoTable({startY:doc.lastAutoTable.finalY+6,head:[['Obrigação','Valor']],body:[['INSS segurados',money57(t.inssEmp)],['IRRF',money57(t.irrf)],['INSS patronal',money57(t.inssPat)],['RAT',money57(t.rat)],['Terceiros',money57(t.terceiros)],['PIS folha',money57(t.pis)],['FGTS',money57(t.fgts)]],theme:'grid',headStyles:{fillColor:[13,43,66]},columnStyles:{1:{halign:'right',fontStyle:'bold'}}});
  doc.addPage('a4','landscape');doc.setFillColor(7,26,44);doc.rect(0,0,297,27,'F');doc.setTextColor(255,255,255);doc.setFontSize(15);doc.text('Composição por colaborador',12,16);doc.autoTable({startY:33,margin:{left:8,right:8},tableWidth:281,head:[['Colaborador','Departamento','Salário','Proventos','Descontos','Líquido','Encargos','Benefícios','Custo']],body:t.rows.map(function(r){return[r.nome,r.departamento,money57(r.salary),money57(r.proventos),money57(r.descontos),money57(r.liquido),money57(r.encargos),money57(r.beneficios),money57(r.custo)]}),foot:[['TOTAL','','',money57(t.proventos),money57(t.descontos),money57(t.liquido),money57(t.encargos),money57(t.beneficios),money57(t.custo)]],theme:'striped',styles:{fontSize:6.3,cellPadding:{top:1.8,right:1.4,bottom:1.8,left:1.4},valign:'middle',overflow:'linebreak'},headStyles:{fillColor:[13,43,66],fontStyle:'bold'},footStyles:{fillColor:[234,242,246],textColor:[7,26,44],fontStyle:'bold'},columnStyles:{0:{cellWidth:73},1:{cellWidth:32},2:{cellWidth:25,halign:'right'},3:{cellWidth:25,halign:'right'},4:{cellWidth:25,halign:'right'},5:{cellWidth:25,halign:'right'},6:{cellWidth:25,halign:'right'},7:{cellWidth:25,halign:'right'},8:{cellWidth:26,halign:'right'}},didParseCell:function(data){if(data.column.index>=2)data.cell.styles.halign='right'}});
...
async function exportExcel57(){
  ...
  var rs=wb.addWorksheet('Resumo');rs.views=[{showGridLines:false}];rs.addRow(['PRÓXIMA FOLHA — RELATÓRIO EXECUTIVO']);rs.addRow(['Projeção',comp57(t.target),'Base',comp57(V57.latest.competencia),'Tabela tributária',TAX57.versao]);rs.addRow([]);header(rs.addRow(['Indicador','Valor']));[['Proventos previstos',t.proventos],['Descontos calculados',t.descontos],['Líquido previsto',t.liquido],['Impostos retidos',t.retained],['Encargos empresa',t.encargos],['Benefícios',t.beneficios],['Custo total',t.custo]].forEach(function(x){rs.addRow(x)});rs.getColumn(1).width=34;rs.getColumn(2).width=18;rs.getColumn(2).numFmt='R$ #,##0.00';
  var wp=wb.addWorksheet('Colaboradores');wp.views=[{showGridLines:false,state:'frozen',ySplit:1}];header(wp.addRow(['Colaborador','Departamento','Vínculo','Salário-base','Dias férias','Proventos','INSS','IRRF','Outros descontos','Líquido','INSS patronal','RAT','Terceiros','PIS','FGTS','Benefícios','Custo']));t.rows.forEach(function(r){wp.addRow([r.nome,r.departamento,r.vinculo,r.salary,r.vacDays,r.proventos,r.inssEmp,r.irrf,r.opsTotal+r.pension+r.manualDiscount,r.liquido,r.inssPat,r.rat,r.terceiros,r.pis,r.fgts,r.beneficios,r.custo])});wp.columns.forEach(function(c,i){c.width=i===0?38:(i<3?20:15);if(i>=3)c.numFmt='R$ #,##0.00'});
  var wi=wb.addWorksheet('Impostos');wi.views=[{showGridLines:false}];header(wi.addRow(['Obrigação','Valor','Regra']));[['INSS segurados',t.inssEmp,'Tabela progressiva 2026 por colaborador'],['IRRF',t.irrf,'Melhor dedução + redução mensal 2026'],['INSS patronal',t.inssPat,'20% quando incidente'],['RAT',t.rat,'Razão da competência-base'],['Terceiros',t.terceiros,'Razão da competência-base'],['PIS folha',t.pis,'Razão da competência-base'],['FGTS',t.fgts,'8% por colaborador; 2% aprendiz']].forEach(function(x){wi.addRow(x)});wi.getColumn(1).width=24;wi.getColumn(2).width=18;wi.getColumn(2).numFmt='R$ #,##0.00';wi.getColumn(3).width=48;
...
function styles57(){
  ...
  '@media(max-width:760px){#rh57-modal{padding:7px}.rh57-modal-card{width:calc(100vw - 14px);max-height:calc(100vh - 14px)}.rh57-form.grid{grid-template-columns:1fr}.rh57-roster-tools{align-items:stretch;flex-direction:column}.rh57-tax-line{grid-template-columns:1fr auto}.rh57-tax-line>span:nth-child(2),.rh57-tax-line>span:nth-child(3){display:none}}';document.head.appendChild(s)
}
function init57(){
  window.RH_FORECAST_V57=true;window.rhV57HandleCapture=handleCapture57;window.rhV57Refresh=refresh57;window.rhV57Snapshot=function(){return V57.snapshot};styles57();ensureWorkforceButton57();enhancePayroll57();
  ...
  document.addEventListener('keydown',function(e){if(e.key==='Escape')closeModal57()});
