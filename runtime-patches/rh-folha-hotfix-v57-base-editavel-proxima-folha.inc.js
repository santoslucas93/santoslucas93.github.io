...
function taxMetric57(key){
  ...
  var body='<p class="rh57-note">'+esc57(note)+'</p><div class="rh57-scroll"><table class="rh57-tax-audit"><thead><tr><th>Colaborador</th><th>Departamento</th><th class="money">Base de cálculo</th><th class="money">Alíquota</th><th>Regra / memória</th><th class="money">Valor</th><th class="money">% do card</th></tr></thead><tbody>'+rows.map(function(x){var d=x.tax;return '<tr><td><b>'+esc57(x.person.nome)+'</b></td><td>'+esc57(x.person.departamento)+'</td><td class="money">'+money57(d.base)+'</td><td class="money">'+pct57(d.rate)+'</td><td>'+esc57(d.rule)+'<small class="rh57-tax-detail">'+esc57(d.detail)+'</small></td><td class="money"><b>'+money57(d.value)+'</b></td><td class="money">'+pct57(total.value?d.value/total.value:0)+'</td></tr>'}).join('')+'</tbody><tfoot><tr><td><b>TOTAL</b></td><td>'+rows.length+' colaboradores</td><td class="money"><b>'+money57(total.base)+'</b></td><td class="money"><b>'+pct57(total.rate)+'</b></td><td>'+esc57(total.rule)+'</td><td class="money"><b>'+money57(total.value)+'</b></td><td class="money"><b>'+(total.value?'100,00%':'—')+'</b></td></tr></tfoot></table></div>';
  openModal57(total.label,'Próxima folha · '+comp57(V57.target),body,1260)
}
function taxGroupModal57(title,keys,total,note){
  var rows=taxAuditRows57(V57.snapshot,keys),body='<p class="rh57-note">'+esc57(note)+'</p><div class="rh57-scroll"><table class="rh57-tax-audit"><thead><tr><th>Colaborador</th><th>Departamento</th><th>Imposto</th><th class="money">Base de cálculo</th><th class="money">Alíquota</th><th>Regra</th><th class="money">Valor</th></tr></thead><tbody>'+rows.map(function(x){return '<tr><td><b>'+esc57(x.person.nome)+'</b></td><td>'+esc57(x.person.departamento)+'</td><td>'+esc57(x.tax.label)+'</td><td class="money">'+money57(x.tax.base)+'</td><td class="money">'+pct57(x.tax.rate)+'</td><td>'+esc57(x.tax.rule)+'</td><td class="money"><b>'+money57(x.tax.value)+'</b></td></tr>'}).join('')+'</tbody><tfoot><tr><td><b>TOTAL DO CARD</b></td><td></td><td></td><td class="money">Bases não somáveis</td><td></td><td></td><td class="money"><b>'+money57(total)+'</b></td></tr></tfoot></table></div>';
...
async function exportPdf57(){
  ...
  doc.addPage('a4','landscape');doc.setFillColor(7,26,44);doc.rect(0,0,297,27,'F');doc.setTextColor(255,255,255);doc.setFontSize(15);doc.text('Memória tributária individual',12,16);doc.setFontSize(7.5);doc.text('Todas as bases de cálculo, alíquotas e valores da projeção '+comp57(t.target),12,22);var audit=taxAuditRows57(t,TAX_KEYS57);doc.autoTable({startY:33,margin:{left:8,right:8,bottom:12},tableWidth:281,head:[['Colaborador','Departamento','Imposto','Base de cálculo','Alíquota','Valor','Regra / memória']],body:audit.map(function(x){return[x.person.nome,x.person.departamento,x.tax.label,money57(x.tax.base),pct57(x.tax.rate),money57(x.tax.value),x.tax.rule+(x.tax.detail?' · '+x.tax.detail:'')]}),theme:'striped',styles:{fontSize:5.9,cellPadding:{top:1.6,right:1.3,bottom:1.6,left:1.3},valign:'middle',overflow:'linebreak'},headStyles:{fillColor:[13,43,66],fontStyle:'bold'},alternateRowStyles:{fillColor:[247,250,252]},columnStyles:{0:{cellWidth:52},1:{cellWidth:28},2:{cellWidth:28},3:{cellWidth:30,halign:'right'},4:{cellWidth:22,halign:'right'},5:{cellWidth:28,halign:'right',fontStyle:'bold'},6:{cellWidth:93}}});
...
async function exportExcel57(){
  ...
  var wa=wb.addWorksheet('Bases por Imposto');wa.views=[{showGridLines:false,state:'frozen',ySplit:1}];header(wa.addRow(['Colaborador','Departamento','Imposto','Base de cálculo','Alíquota efetiva/aplicada','Valor','Regra','Memória adicional']));taxAuditRows57(t,TAX_KEYS57).forEach(function(x){wa.addRow([x.person.nome,x.person.departamento,x.tax.label,x.tax.base,x.tax.rate,x.tax.value,x.tax.rule,x.tax.detail])});wa.getColumn(1).width=38;wa.getColumn(2).width=22;wa.getColumn(3).width=22;wa.getColumn(4).width=18;wa.getColumn(4).numFmt='R$ #,##0.00';wa.getColumn(5).width=25;wa.getColumn(5).numFmt='0.00%';wa.getColumn(6).width=18;wa.getColumn(6).numFmt='R$ #,##0.00';wa.getColumn(7).width=34;wa.getColumn(8).width=42;
