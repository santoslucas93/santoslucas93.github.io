...
function taxBaseMemoryHtml57(memory){
  var lines=memory.items.map(function(x){return '<div><span><i>'+esc57(x.sign)+'</i>'+esc57(x.label)+'</span><b>'+money57(x.value)+'</b></div>'}).join('');
  var out=memory.excluded.map(function(x){return '<div><span><i>×</i>'+esc57(x.label)+'</span><b>'+money57(x.value)+'</b></div>'}).join('');
  return '<details class="rh57-base-memory"><summary>Ver composição</summary><section>'+lines+'<div class="total"><span>Base de cálculo</span><b>'+money57(memory.base)+'</b></div>'+(out?'<p>Fora da base</p>'+out:'')+'<small>'+esc57(memory.equation)+'</small></section></details>'
}
...
function taxMetric57(key){
  ...
  var body='<p class="rh57-note">'+esc57(note)+'</p><div class="rh57-scroll"><table class="rh57-tax-audit"><thead><tr><th>Colaborador</th><th>Departamento</th><th class="money">Base de cálculo</th><th>Formação da base</th><th class="money">Alíquota</th><th>Regra / memória</th><th class="money">Valor</th><th class="money">% do card</th></tr></thead><tbody>'+rows.map(function(x){var d=x.tax;return '<tr><td><b>'+esc57(x.person.nome)+'</b></td><td>'+esc57(x.person.departamento)+'</td><td class="money">'+money57(d.base)+'</td><td>'+taxBaseMemoryHtml57(d.memory)+'</td><td class="money">'+pct57(d.rate)+'</td><td>'+esc57(d.rule)+'<small class="rh57-tax-detail">'+esc57(d.detail)+'</small></td><td class="money"><b>'+money57(d.value)+'</b></td><td class="money">'+pct57(total.value?d.value/total.value:0)+'</td></tr>'}).join('')+'</tbody><tfoot><tr><td><b>TOTAL</b></td><td>'+rows.length+' colaboradores</td><td class="money"><b>'+money57(total.base)+'</b></td><td>Bases formadas individualmente</td><td class="money"><b>'+pct57(total.rate)+'</b></td><td>'+esc57(total.rule)+'</td><td class="money"><b>'+money57(total.value)+'</b></td><td class="money"><b>'+(total.value?'100,00%':'—')+'</b></td></tr></tfoot></table></div>';
  openModal57(total.label,'Próxima folha · '+comp57(V57.target),body,1260)
}
function taxGroupModal57(title,keys,total,note){
  var rows=taxAuditRows57(V57.snapshot,keys),body='<p class="rh57-note">'+esc57(note)+'</p><div class="rh57-scroll"><table class="rh57-tax-audit"><thead><tr><th>Colaborador</th><th>Departamento</th><th>Imposto</th><th class="money">Base de cálculo</th><th>Formação da base</th><th class="money">Alíquota</th><th>Regra</th><th class="money">Valor</th></tr></thead><tbody>'+rows.map(function(x){return '<tr><td><b>'+esc57(x.person.nome)+'</b></td><td>'+esc57(x.person.departamento)+'</td><td>'+esc57(x.tax.label)+'</td><td class="money">'+money57(x.tax.base)+'</td><td>'+taxBaseMemoryHtml57(x.tax.memory)+'</td><td class="money">'+pct57(x.tax.rate)+'</td><td>'+esc57(x.tax.rule)+'</td><td class="money"><b>'+money57(x.tax.value)+'</b></td></tr>'}).join('')+'</tbody><tfoot><tr><td><b>TOTAL DO CARD</b></td><td></td><td></td><td class="money">Bases não somáveis</td><td>Composição individual</td><td></td><td></td><td class="money"><b>'+money57(total)+'</b></td></tr></tfoot></table></div>';
...
function styles57(){
  ...
  '.rh57-tax-audit{min-width:1260px!important}.rh57-tax-audit th,.rh57-tax-audit td{vertical-align:middle}.rh57-tax-detail{display:block;margin-top:3px;color:var(--muted);font-size:9px;line-height:1.35}.rh57-base-memory{min-width:180px}.rh57-base-memory summary{color:var(--gold-2);font-size:10px;font-weight:850;cursor:pointer;white-space:nowrap}.rh57-base-memory section{display:grid;gap:5px;margin-top:7px;padding:8px;border:1px solid var(--line-soft);border-radius:8px;background:var(--surface-2)}.rh57-base-memory section>div{display:flex;justify-content:space-between;gap:12px}.rh57-base-memory section span{display:flex;gap:6px;line-height:1.3}.rh57-base-memory section i{width:10px;color:var(--gold-2);font-style:normal;font-weight:900}.rh57-base-memory section b{white-space:nowrap}.rh57-base-memory section .total{padding-top:5px;border-top:1px solid var(--line);font-weight:900}.rh57-base-memory section p{margin:3px 0 0;color:var(--muted);font-size:8px;font-weight:900;text-transform:uppercase}.rh57-base-memory section small{padding-top:4px;color:var(--muted);line-height:1.35}.rh57-form{display:grid;gap:12px}.rh57-form.grid{grid-template-columns:1fr 1fr}.rh57-form label{display:grid;gap:5px;color:var(--muted);font-size:10px;font-weight:800;text-transform:uppercase}.rh57-form input,.rh57-form select,.rh57-roster-tools input{height:40px;padding:0 10px;border:1px solid var(--line-soft);border-radius:9px;background:var(--surface-2);color:var(--text)}.rh57-form-note,.rh57-form .button{grid-column:1/-1}.rh57-roster-tools{display:flex;justify-content:space-between;gap:12px;align-items:center;margin-bottom:12px}.rh57-roster-tools input{width:min(390px,100%)}.rh57-roster-tools>div{display:flex;gap:8px;align-items:center;flex-wrap:wrap}.rh57-roster-tools .status{white-space:nowrap}'+
