...
function openGuideCard(key){var r=guideRows().find(function(x){return x.key===key});if(!r)return;if(typeof rhInterOpen==='function')rhInterOpen(r.label,'COMPOSIÇÃO EXATA DA GUIA',['Obrigação','Base','Valor'],[[r.label,money(r.base),money(r.value)]],['TOTAL DO CARD','',money(r.value)],guideLabel()+(r.estimated?' · estimativa gerencial':' · RPA excluído do IRRF da folha quando identificado'));}
function bindGuideCards(){Array.from(document.querySelectorAll('#rh40-guide-grid .rh40-guide-card')).forEach(function(card){var btn=card.querySelector('[data-rh42-guide]');if(!btn)return;card.classList.add('rh-clickable-kpi');card.setAttribute('role','button');card.setAttribute('tabindex','0');card.onclick=function(e){if(e&&e.target&&e.target.closest&&e.target.closest('button'))return;openGuideCard(btn.dataset.rh42Guide)};card.onkeydown=function(e){if(e&&e.target&&e.target.closest&&e.target.closest('button'))return;if(e.key==='Enter'||e.key===' '){e.preventDefault();openGuideCard(btn.dataset.rh42Guide)}};});}
function syncGuides(){var grid=E('rh40-guide-grid');if(grid){grid.innerHTML=guideCards();bindGuideCards()}var ref=E('rh42-guide-ref');if(ref)ref.textContent=guideLabel();scheduleFit(20)}
...
function init(){styles();wrapRender();refresh();setTimeout(refresh,500);setTimeout(refresh,1200);document.addEventListener('click',function(e){var g=e.target&&e.target.closest&&e.target.closest('[data-rh42-guide]');if(g){e.preventDefault();exportGuide(g.dataset.rh42Guide).catch(function(err){try{toast(err.message||String(err),true)}catch(x){}});return}if(e.target&&e.target.closest&&e.target.closest('[data-plan-tab],.nav-item')){setTimeout(refresh,60);setTimeout(refresh,180)}} ,true);['rh-period-year','rh-period-month'].forEach(function(id){var x=E(id);if(x)x.addEventListener('change',function(){setTimeout(refresh,160)})});window.addEventListener('resize',function(){scheduleFit(60)});if(window.ResizeObserver){V.resize=new ResizeObserver(function(){scheduleFit(50)});document.querySelectorAll('.kpi,.rh40-guide-card').forEach(function(x){if(x.closest('#page-planejamento'))return;try{V.resize.observe(x)}catch(e){}})}}
