(function(){
  ...
  async function loadCompetences(selectId){
    S.competencias=await api('rh_competencias?select=*&order=competencia.desc');var sel=$('competencia-select');sel.innerHTML=S.competencias.length?S.competencias.map(function(c){return '<option value="'+c.id+'">'+esc(formatCompetence(c.competencia))+'</option>';}).join(''):'<option value="">Nenhuma importação</option>';
  ...
  function renderKpis(){var c=S.competencia,r=c.resumo||{};$('kpi-proventos').textContent=fmt(c.proventos);$('kpi-descontos').textContent=fmt(c.descontos);$('kpi-liquido').textContent=fmt(c.liquido);$('kpi-pessoas').textContent=nfmt(r.pessoas||S.pessoas.length);$('kpi-vinculos').textContent=(r.empregados||0)+' CLT · '+(r.estagiarios||0)+' estagiários';$('payroll-kpis').innerHTML=[['Proventos',c.proventos],['Descontos',c.descontos],['Líquido',c.liquido],['FGTS',c.valor_fgts]].map(function(x){return '<div class="kpi"><span>'+x[0]+'</span><strong>'+fmt(x[1])+'</strong><small>'+formatCompetence(c.competencia)+'</small></div>';}).join('');}
  function renderPeople(){var q=cleanSearch($('employee-search').value),rows=S.pessoas.filter(function(p){return !q||cleanSearch([p.nome,p.matricula,p.departamento,p.cargo].join(' ')).indexOf(q)>=0;});$('employee-rows').innerHTML=rows.length?rows.map(function(p){return '<tr><td><div class="row-person"><span class="avatar">'+esc(initials(p.nome))+'</span><span><b>'+esc(p.nome)+'</b><small>'+esc(p.cargo||p.cpf_mascarado||'')+'</small></span></div></td><td>'+esc(p.matricula)+'</td><td>'+esc(p.vinculo||'—')+'</td><td>'+esc(departmentName(p.departamento))+'</td><td><span class="status '+(/demit/i.test(p.situacao||'')?'danger':'success')+'">'+esc(p.situacao||'—')+'</span></td><td class="money"><b>'+fmt(p.liquido)+'</b></td><td><button class="detail-button" data-person="'+esc(p.id)+'">Ver composição</button></td></tr>';}).join(''):emptyRow(7,'Nenhum registro individual disponível para este perfil.');bindPersonButtons();}
  ...
  async function handlePdf(file){if(!file)return;if(file.size>25*1024*1024){toast('O PDF ultrapassa o limite de 25 MB.',true);return;}try{toast('Preparando o leitor de PDF…');await loadLibrary('pdf');toast('Lendo e validando o PDF…');var result=await RHParser.extractPdf(file);if(!result.competencia||!result.competencia.competencia||!result.colaboradores.length)throw new Error('O relatório não pôde ser reconhecido. Verifique se o PDF possui texto selecionável.');showPreview(result);}catch(e){toast(e.message,true);}}
  async function handleExcel(file){if(!file)return;try{toast('Preparando o leitor de Excel…');await loadLibrary('xlsx');toast('Lendo e validando a planilha…');showPreview(await RHParser.parseExcel(file));}catch(e){toast(e.message,true);}}
...
async function confirmImport(){if(!S.preview)return;var btn=$('confirm-import');btn.disabled=true;btn.textContent='Importando…';try{var id=await rpc('rh_importar_folha',{p_payload:buildRpcPayload(S.preview)});toast('Competência importada com sucesso.');S.preview=null;$('import-preview').hidden=true;await loadCompetences(id);go('visao');}catch(e){toast('Não foi possível importar: '+e.message,true);}finally{btn.disabled=false;btn.textContent='Confirmar importação';}}
  ...
  function openPerson(id){var p=S.pessoas.find(function(x){return x.id===id;});if(!p)return;$('employee-modal-title').textContent=p.nome;$('employee-modal-summary').innerHTML=[['Matrícula',p.matricula],['Proventos',fmt(p.proventos)],['Descontos',fmt(p.descontos)],['Líquido',fmt(p.liquido)]].map(function(x){return '<div><span>'+x[0]+'</span><strong>'+esc(x[1])+'</strong></div>';}).join('');$('employee-modal-rows').innerHTML=p.lancamentos.length?p.lancamentos.map(function(x){return '<tr><td><b>'+esc((x.rubrica_codigo||'')+' '+x.rubrica_nome)+'</b></td><td><span class="status">'+esc(x.tipo)+'</span></td><td class="money">'+nfmt(x.referencia)+'</td><td class="money"><b>'+fmt(x.valor)+'</b></td></tr>';}).join(''):emptyRow(4,'Sem rubricas individuais disponíveis.');$('employee-modal').hidden=false;}
  function closeModal(){$('employee-modal').hidden=true;}

  function askAI(question){question=String(question||'').trim();if(!question)return;addMessage(question,'user');var q=cleanSearch(question),r=S.competencia&&S.competencia.resumo||{},d=departments().slice().sort(function(a,b){return b.liquido-a.liquido;}),e=S.competencia&&S.competencia.encargos||{},answer,trace,view='visao',composition=false;if(!S.competencia){answer='Ainda não existe uma competência importada. Use o módulo de Importação para carregar o primeiro arquivo.';view='importacao';trace=null;}else if(/liquido|líquido|total da folha/.test(q)){answer='O líquido da folha de '+formatCompetence(S.competencia.competencia)+' é '+fmt(S.competencia.liquido)+'.';trace='liquido';composition=true;}else if(/departamento|area|área|maior custo/.test(q)){answer=d.length?'O departamento com maior custo líquido é '+d[0].nome+', com '+fmt(d[0].liquido)+'.':'A competência não possui rateio por departamento.';trace='departamentos';view='rateio';}else if(/quant|pessoa|colaborador|headcount/.test(q)){answer='A folha possui '+nfmt(r.pessoas||S.pessoas.length)+' pessoas: '+nfmt(r.empregados)+' empregados e '+nfmt(r.estagiarios)+' estagiários.';trace='headcount';view='colaboradores';composition=true;}else if(/fgts/.test(q)){answer='O FGTS da competência é '+fmt(e.valor_fgts||S.competencia.valor_fgts)+', sobre base de '+fmt(e.base_fgts||S.competencia.base_fgts)+'.';trace='encargos';view='encargos';}else if(/desconto/.test(q)){answer='Os descontos totalizam '+fmt(S.competencia.descontos)+'. A composição está disponível em Rubricas.';trace='descontos';view='rubricas';composition=true;}else{answer='Posso responder sobre líquido, descontos, FGTS, quantidade de pessoas e custos por departamento. Quando houver vínculo verificável, mostro a origem e a composição.';trace=null;}addAnswer(answer,trace,view,composition);}
  ...
  function revalidateInBackground(){rpc('meu_acesso').then(function(fresh){if(!validAccess(fresh)){clearSession();location.href='/?acesso_negado=rh';return;}ACCESS=fresh;saveAccessSnapshot(fresh);setupPermissions();}).catch(function(){/* A RLS do banco continua sendo a autoridade para todos os dados e ações. */});}

  async function start(){
    window.__rhBootReady=true;bind();applyTheme();offerCentralValidation();try{setGate('Carregando a configuração segura…');var configResponse=await fetchTimed('/api/config',{},10000);if(!configResponse.ok)throw new Error('Configuração indisponível.');CFG=await configResponse.json();SES=loadSession();if(!SES)throw new Error('Entre pela Central de Gestão para acessar este módulo.');var expiry=Number(SES.expires_at)||0;if(expiry&&expiry<1e12)expiry*=1000;if(!expiry||Date.now()>expiry-60000){setGate('Renovando sua sessão…');SES=await refresh(SES);}if(!SES)throw new Error('Sua sessão expirou. Entre novamente pela Central de Gestão.');var cachedAccess=loadAccessSnapshot();if(validAccess(cachedAccess)){setGate('Abrindo RH & Folha…');await openModule(cachedAccess);revalidateInBackground();return;}setGate('Verificando seu acesso ao RH & Folha…');try{ACCESS=await rpc('meu_acesso');}catch(firstError){setGate('A validação demorou. Fazendo uma segunda tentativa…');await delay(650);ACCESS=await rpc('meu_acesso');}if(!validAccess(ACCESS))throw new Error('Seu usuário não possui acesso ao módulo RH & Folha.');saveAccessSnapshot(ACCESS);await openModule(ACCESS);}catch(e){setGate(e&&e.message?e.message:'Não foi possível validar o acesso. Tente novamente.');$('gate-home').hidden=false;}
  }
